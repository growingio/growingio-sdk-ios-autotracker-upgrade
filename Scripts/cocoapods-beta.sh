...
POD_BETA_VERSOIN=`cat GrowingAnalytics-upgrade.podspec | grep 's.version\s*=' | grep -Eo '[0-9]+.[0-9]+.[0-9]+'-beta`
BETA='beta'
...
echo y | pod trunk delete GrowingAnalytics-upgrade $POD_BETA_VERSOIN
